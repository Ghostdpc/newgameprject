## 职责：开发调试可视化（碰撞体 / 骨骼线 / 角色数据面板），热键 F1/F2/F3 切换
## 碰撞体与骨骼线均自绘并开启 no_depth_test，穿透模型可见
## F3 数据面板：遍历所有玩家，实时显示角色持有道具/速度/状态等运行时数据
## F4 暂停/恢复倒计时（冻结结算用）
## F5 对 P1 注入道具并立即使用（测试道具效果用）
## F6 打开/关闭道具调试列表（点击道具名立即对 P1 触发效果）
## F7 清除场上全部陷阱，在 P1 脚下强制生成一个香蕉皮（视觉排查用）
## F9 在 Output 按世界 AABB 体积排序输出碰撞体，定位超大或偏移的形状
...
## F1 碰撞体三态循环：0=关 → 1=仅玩家 → 2=全部(含场景/道具) → 0
...
func _toggle_timer_pause() -> void:
	...
	# time_scale = 0 冻结，1 恢复；不触碰 stage_time_remaining 值
...
## 遍历所有玩家身上的 Skeleton3D 画骨线（父骨→子骨）
...
## 画碰撞体线框：PLAYERS=只角色，ALL=整棵树全部（含场景静态体/道具）
...
## 构建 overlay：CanvasLayer → HBoxContainer（每个玩家一个 Panel + Label）
...
## F7：清除场上所有陷阱，在 P1 脚下重新生成一个香蕉皮（视觉排查用）
func _force_respawn_banana() -> void:
	...
	if p1 == null:
		...
		return
	# 3. 直接构建 TrapInstance 并放在 P1 前方 1m 处（y 贴地）
...
## 建一个带背景说明的 Label（值文字由对应 _on_*_changed 填）
