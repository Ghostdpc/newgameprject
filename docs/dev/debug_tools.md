...
- 切換 `GameManager.time_scale` 在 `0.0`（凍結）和 `1.0`（正常）之間
- 倒計時數值保持不變，結算不會觸發，可用於截圖 / 測試道具效果
- 再次按 F4 恢復正常流速
...
- 每按一次，按以下順序循環向 **P1** 注入一個道具並立即使用：

```
energy_drink → banana_peel → fast_forward_crank → slow_hourglass
→ time_battery → time_scissors → camera_remote → (循環)
```

- 直接調用 `p1.use_held_item()`，走完整的 `ItemSystem.use_item()` 鏈路
- `camera_remote` 當前為預留接口，會在 Console 打印 warning，不影響其他功能
