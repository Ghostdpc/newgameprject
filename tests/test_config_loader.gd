...
func test_load_nonexistent_config_returns_empty_dict() -> void:
	var cfg: Dictionary = _loader.load_config("__nonexistent__")
	assert_true(cfg is Dictionary)
	assert_true(cfg.is_empty(), "不存在的配置應回傳空 Dictionary")
...
func test_cache_returns_same_reference() -> void:
	...
	assert_eq(cfg1, cfg2, "第二次讀取應命中快取回傳相同內容")

# --- get_value ---

func test_get_value_returns_battle_duration() -> void:
	var val: Variant = _loader.get_value("game_flow", "battle_duration", 0.0)
	assert_eq(float(val), 15.0, "battle_duration 應為 15.0")

func test_get_value_missing_key_returns_default() -> void:
	var val: Variant = _loader.get_value("game_flow", "__no_such_key__", 99.0)
	assert_eq(val, 99.0, "缺失鍵應回傳默認值 99.0")

# --- GameConfig ---

func test_game_config_battle_duration_from_json() -> void:
...
func test_game_config_skipped_stages_are_zero() -> void:
	var cfg: GameConfig = GameConfig.new()
	cfg.load_from_json()
	assert_eq(cfg.theme_announce_duration, 0.0, "theme_announce 應為 0")
	assert_eq(cfg.grab_clothes_duration,   0.0, "grab_clothes 應為 0")
	assert_eq(cfg.scoring_duration,        0.0, "scoring 應為 0")
