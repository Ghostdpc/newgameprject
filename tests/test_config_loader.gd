...
func test_load_existing_config_returns_dictionary() -> void:
	var cfg := _loader.load_config("game_flow")
...
func test_load_nonexistent_config_returns_empty_dict() -> void:
	var cfg := _loader.load_config("__nonexistent__")
...
func test_cache_returns_same_instance() -> void:
	var cfg1 := _loader.load_config("game_flow")
	var cfg2 := _loader.load_config("game_flow")
...
func test_get_value_returns_correct_value() -> void:
	...
	assert_eq(val, 15.0, "battle_duration 應為 15.0")
