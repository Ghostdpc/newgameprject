...
func test_human_anim_mapping() -> void:
	...
	if ap:
		...
		assert_true(anim_move.ends_with("Walk"), "移动用 Walk 动画")
		assert_true(anim_idle.ends_with("Idle") or anim_idle.ends_with("Idle_001"), "待机用 Idle 动画")
		assert_true(anim_jump.ends_with("jump"), "跳跃用 jump 动画")
